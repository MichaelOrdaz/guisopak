  ...
  foreach ($unidad as $key=> $grupo) {
    ...
    foreach ($grupo as $key=>$fecha) {
      ...
      foreach ($fecha as $recetas) {
        ...
        $sql = "SELECT art.idArticulo, art.nombre, art.unidad, reart.cantidad, art.costo FROM receta AS re JOIN recetaart AS reart ON re.idReceta=reart.receta JOIN articulo AS art ON art.idArticulo=reart.articulo WHERE re.nombre = '{$recetas[0]->receta}'";
        $rslt = $db->query($sql);
        ...
        //   //si la receta tiene articulos que dibujar entonces dibujo su header
          ...
          while($articulo = $rslt->fetch_object() ){
            ...
            $sheet->setCellValue("D{$indexRow}", $articulo->cantidad);
