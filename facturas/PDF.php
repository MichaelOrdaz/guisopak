...
$sql = "SELECT cliente FROM oc WHERE idOC = '{$orden}' AND status = '2' LIMIT 1";
$r = $db->query($sql);
...
$r = $db->query("SELECT articulo, (SELECT nombre FROM articulo WHERE idArticulo = articulo LIMIT 1 ) AS nombre, SUM(cantidad) AS cantidad, costoU FROM bomoc AS bom WHERE OC = '{$orden}' GROUP BY articulo");
